...
class ContabilidadServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->bind(
            'Modules\Contabilidad\Repositories\TipoCuentaRepository',
            function () {
                $repository = new \Modules\Contabilidad\Repositories\Eloquent\EloquentTipoCuentaRepository(new \Modules\Contabilidad\Entities\TipoCuenta());

                if (! config('app.cache')) {
                    return $repository;
                }

                return new \Modules\Contabilidad\Repositories\Cache\CacheTipoCuentaDecorator($repository);
            }
        );
        $this->app->bind(
